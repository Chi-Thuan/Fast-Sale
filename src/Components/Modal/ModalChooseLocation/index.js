...
import { _convertToSlug } from '../../../Utils/utils'
import Icon from 'react-native-vector-icons/dist/FontAwesome';
import PROVINCES from '../../../Constant/Location/Provinces'

class ModelChooseLocation extends Component {
    ...
    __renderListProvinces = () => {
        ...
        for (let index in PROVINCES) {
            if (PROVINCES[index].slug.toLowerCase().indexOf(_convertToSlug(txt_search)) > -1) {
                listItem.push(
                    <OptionItem 
                        key={index}
                        isClicked={this.state.currChoose_code == PROVINCES[index].code ? true : false}
    ...
    render() {
        ...
        const { isVisible, closeModal, actionChoose } = this.props
...
export default ModelChooseLocation